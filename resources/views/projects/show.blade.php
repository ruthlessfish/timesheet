<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <x-primary-button type="button" onclick="window.location='{{ route('projects.edit', $project) }}'">
                Edit Project
            </x-primary-button>
        </div>
    </x-slot>
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="font-medium text-gray-900 dark:text-gray-100">{{ $project->hourly_rate ? '$' .
                                number_format($project->hourly_rate, 2) : 'Using client rate' }}</p>
                        </div>
                        <div>
                            <span class="text-sm text-gray-500 dark:text-gray-400">Budget:</span>
                            <p class="font-medium text-gray-900 dark:text-gray-100">{{ $project->budget ? '$' .
                                number_format($project->budget, 2) : 'N/A' }}</p>
                        </div>
                        <div>
                            <span class="text-sm text-gray-500 dark:text-gray-400">Start Date:</span>
                            <p class="font-medium text-gray-900 dark:text-gray-100">{{ $project->start_date ?
                                $project->start_date->format('M d, Y') : 'N/A' }}</p>
                        </div>
                        <div>
                            <span class="text-sm text-gray-500 dark:text-gray-400">End Date:</span>
                            <p class="font-medium text-gray-900 dark:text-gray-100">{{ $project->end_date ?
                                $project->end_date->format('M d, Y') : 'N/A' }}</p>
                        </div>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        ...
                                        @else
                                        <span
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300">No</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
