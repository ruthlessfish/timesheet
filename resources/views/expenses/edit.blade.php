    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form id="edit-expense-form" action="{{ route('expenses.update', $expense) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <select name="client_id" id="client_id" required
                                    ...
                                    <option value="{{ $client->id }}" {{ old('client_id', $expense->client_id) ==
                                        $client->id ? 'selected' : '' }}>
                                        {{ $client->name }}
                                    </option>
                                    @endforeach
                    ...
                    <div class="mt-6 flex justify-between space-x-3">
                        <x-delete-button :url="route('expenses.destroy', $expense)"
                            confirm-text="Are you sure you want to delete this expense?" />
                        <div>
                            <x-secondary-button type="button"
                                onclick="window.location='{{ route('expenses.show', $expense) }}'">
                                Cancel
                            </x-secondary-button>
                            <x-primary-button type="submit" form="edit-expense-form">
                                Update Expense
                            </x-primary-button>
                        </div>
                    </div>
