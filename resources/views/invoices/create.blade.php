                    ...
                    <form action="{{ route('invoices.store') }}" method="POST" id="invoice-form">
                        ...
                        <div class="space-y-6">
                                ...
                                <div
                                    ...
                                    <div class="flex items-start space-x-3 p-2 hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <input type="checkbox" name="time_entries[]" value="{{ $entry->id }}"
                                            id="entry_{{ $entry->id }}" {{ in_array($entry->id, old('time_entries', []))
                                        ? 'checked' : '' }}
                                        ...
                                        <div class="text-sm font-medium text-gray-900 dark:text-gray-100">
                                            ${{ number_format(($entry->duration / 60) * ($entry->hourly_rate ??
                                            $entry->project->hourly_rate ?? $entry->project->client->hourly_rate ?? 0),
                                            2) }}
                                        </div>
                                    </div>
                                    @endforeach
                            ...
                            <div
                                ...
                                <p class="text-sm text-yellow-800 dark:text-yellow-400">No unbilled time entries found
                                    for this client.</p>
                            </div>
                            @else
                            <div
                                class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-md p-4">
                                <p class="text-sm text-blue-800 dark:text-blue-400">Select a client to view unbilled
                                    time entries.</p>
                            </div>
                            @endif
                        </div>
