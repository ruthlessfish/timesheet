<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="space-x-2">
                <a href="{{ route('invoices.pdf', $invoice) }}" class="bg-green-600 dark:bg-green-500 hover:bg-green-700 dark:hover:bg-green-600 text-white px-4 py-2 rounded">
                    Download PDF
                </a>
                <a href="{{ route('invoices.edit', $invoice) }}" class="bg-blue-600 dark:bg-blue-500 hover:bg-blue-700 dark:hover:bg-blue-600 text-white px-4 py-2 rounded">
                    Edit Invoice
                </a>
            </div>
        </div>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            @if(session('success'))
                <div class="mb-4 bg-green-100 dark:bg-green-900/30 border border-green-400 dark:border-green-700 text-green-700 dark:text-green-400 px-4 py-3 rounded">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8">
                    ...
                    <div class="grid grid-cols-2 gap-8 mb-8">
                        ...
                        <div class="text-right">
                            <h3 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase mb-2">Invoice Details</h3>
                            <div class="text-gray-900 dark:text-gray-100">
                                <p><span class="text-gray-600 dark:text-gray-400">Issue Date:</span> {{ $invoice->issue_date->format('M d, Y') }}</p>
                                <p><span class="text-gray-600 dark:text-gray-400">Due Date:</span> {{ $invoice->due_date->format('M d, Y') }}</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-900">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Hours</th>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr>
                                    <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                        {{ $item->description }}
                                        @if($item->timeEntry)
                                            <span class="text-xs text-gray-500 dark:text-gray-400">({{ $item->timeEntry->start_time->format('M d, Y') }})</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 text-sm text-right text-gray-900 dark:text-gray-100">{{ number_format($item->quantity, 2) }}</td>
                                    <td class="px-6 py-4 text-sm text-right text-gray-900 dark:text-gray-100">${{ number_format($item->rate, 2) }}</td>
                                    <td class="px-6 py-4 text-sm text-right font-medium text-gray-900 dark:text-gray-100">${{ number_format($item->amount, 2) }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    <div class="flex justify-end mb-8">
                        <div class="w-64">
                            ...
                            <div class="flex justify-between py-3 text-lg font-bold border-t border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100">
                                <span>Total:</span>
